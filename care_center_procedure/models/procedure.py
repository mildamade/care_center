...
class ProcedureProcedure(models.Model):
    ...
    child_ids = fields.One2many(
        "procedure.procedure", "parent_id", string="Checklist"
    )
    ...
    def _limit_depth(self):
        if self.parent_id and self.parent_id.parent_id:
            raise ValidationError(
                "%s is a checklist item and cannot be a parent procedure"
                % self.parent_id.name
            )
    ...
    def _compile_documentation(self):
        ...
        for procedure in self:
            ...
            for checklist in checklists:
                docs.append(
                    "<h3>%s</h3> %s" % (checklist.name, checklist.description)
                )
...
class ProcedureAssignment(models.Model):
    ...
    parent_id = fields.Many2one(
        "procedure.procedure", "Procedure", ondelete="cascade", required=False
    )
    procedure_id = fields.Many2one(
        "procedure.procedure", "Checklist", ondelete="cascade", required=True
    )
    sequence = fields.Integer()
    task_id = fields.Many2one(
        "project.task", "Ticket", ondelete="cascade", required=False, index=True
    )
    description = fields.Html("Description", related="procedure_id.description")
    documentation = fields.Html(
        "Documentation", related="procedure_id.documentation"
    )
    ...
    def show_documentation(self):
        ...
        """
        form = self.env.ref(
            "care_center_procedure.view_procedure_documentation_form"
        )
    ...
    def set_parent_procedure_status(self):
        ...
        unfinished_checklist = self.search(
            [
        ...
        procedure_assignment.write(
            {"status": "working" if unfinished_checklist else "done"}
        )
    ...
    def create(self, vals):
        ...
        if assignment.procedure_id.planned_hours and assignment.task_id:
            assignment.task_id.write(
                {
                    "planned_hours": assignment.task_id.planned_hours
                    + assignment.procedure_id.planned_hours
                }
            )
        return assignment
